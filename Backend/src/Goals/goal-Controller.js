...
const addProgress = async (req, res) => {
    try {
        const goalId = req.params.goalId || req.params.id  // ✅ already fixed

        const { amount } = req.body

        if (!goalId) {
            return res.status(400).json({ success: false, message: "Goal ID missing" })
        }
        ...
        const goalDoc = await goalCollection.doc(goalId).get()  // ✅ was `id`
        if (!goalDoc.exists) {
        ...
        await goalCollection.doc(goalId).update(updateData)  // ✅ was `id`
        const updatedProgress = await goalCollection.doc(goalId).get()  // ✅ was `id`

        res.status(200).json({
            success: true,
            message: "Progress Updated Successfully",
            data: { id: updatedProgress.id, ...updatedProgress.data() }
        })
    }
    catch (error) {
        res.status(500).json({ success: false, message: "Error Adding Progress", error: error.message })
    }
}
