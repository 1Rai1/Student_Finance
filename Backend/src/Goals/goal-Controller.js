...
const createGoal = async (req,res) => {
    try{
        ...
        const goalData = {
            ...
            status:'active',// active || completed otherwise deleted,
            progress: 0,
...
const updateGoals = async (req,res) => {
    ...
    const goalDoc = await goalCollection.doc(goalCollection).get()
    if(!goalDoc.exists){
        return res.status(404).json({
            success:false,
            message:"No Goals Found"
    ...
    const newCurrentAmount = updateData.targetAmount !== undefined ? updateData.currentAmount: currentData.currentAmount
    ...
    //Get update
    const updateDoc = await goalCollection.doc(id).get()
...
const addProgres = async (req,res) =>{
    try{
        const {goalId} = req.params
        ...
        if(newCurrentAmount >= goalData.targetAmount && goalData.status === 'active'){
            updateData.status = 'completed'
            updateData.completedAt = new Date().toDateString()
        }

        //update Progress
        await goalCollection.doc(goalId).update(updateData)
        //get updated
        const updatedProgress = await goalCollection.doc(id).get()
         ...
         res.status(200).json({
            ...
            data:{
                ...
                goals:{
                    ...
                    completedGoal: goals.filter(g => g.status === 'completed').length 
                }
            }
         })
    }
